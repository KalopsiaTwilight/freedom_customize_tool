...
import { setupDbIpc, setupMiscIpc, setupPatchingIpc, setUpStoreIpc, setupSharpIpc } from './ipc';
import { CallGetExpressUriChannel, OnFirstStartChannel, OnOpenChannel, OnSaveChannel } from './ipc/channels';
import { ExpressSendWoWDirType, isPortInfoMessage } from './ipc/expressIPC';
...
let expressAppProcess: Electron.UtilityProcess;
...
async function setupIpc() {
  ...
  if (!settings.freedomWoWRootDir) {
    ...
    expressAppProcess.postMessage({ type: ExpressSendWoWDirType, data: settings.freedomWoWRootDir })
  }
}
...
async function createWindow(): Promise<void> {
  ...
  try {
    ...
    expressAppProcess = utilityProcess.fork(expressPath, [], {
      stdio: "pipe",
    });
    expressAppProcess.on('message', (msg) => {
      if (isPortInfoMessage(msg)) {
        expressPort = msg.data;

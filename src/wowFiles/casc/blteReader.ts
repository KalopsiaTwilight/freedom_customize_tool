...
import * as tactKeys from "./tactKeys";
...
export class BLTEReader extends BufferWrapper {
     ...
     */
    static check(data: BufferWrapper) {
    ...
    constructor(buf: BufferWrapper, hash: string, partialDecrypt = false) {
        ...
        for (let i = 0; i < numBlocks; i++) {
            const block: BLTEBlock = { CompSize: 0, DecompSize: 0, Hash: "" };
            if (headerSize !== 0) {
     ...
     * @param {number} index 
     */
    _handleBlock(block: BufferWrapper, blockEnd: number, index: number) {
     ...
     * @param {number} index 
     */
    _decompressBlock(data: BufferWrapper, blockEnd: number, index: number) {
     ...
     * @param {number} index 
     */
    _decryptBlock(data: BufferWrapper, blockEnd: number, index: number) {
     ...
     * @param {number} blockEnd
     */
    _writeBufferBLTE(buf: BufferWrapper, blockEnd: number,) {
